...
int main() {
    string ecoli_path = "ecoli/ecoli.txt";
    ifstream ecoli_file(ecoli_path);
    string ecoli_content((istreambuf_iterator<char>(ecoli_file)), (istreambuf_iterator<char>()));
    ecoli_file.close();

    string ecoliMapping_path = "ecoli-mapping.txt";
    ifstream ecoliMapping_file(ecoliMapping_path);
    string ecoliMapping_content((istreambuf_iterator<char>(ecoliMapping_file)), (istreambuf_iterator<char>()));
    ecoliMapping_file.close();

    unordered_map<string, string> ecoliMappingDictionary;
    vector<string> mappingArray;
    size_t pos = 0;
    string delimiter = ",";
    while ((pos = ecoliMapping_content.find(delimiter)) != string::npos) {
        string mapPair = ecoliMapping_content.substr(0, pos);
        mappingArray.push_back(mapPair);
        ecoliMapping_content.erase(0, pos + delimiter.length());
    }
    for (const string& mapPair : mappingArray) {
        ...
        ecoliMappingDictionary[key] = value;
    }

    unordered_map<int, string> ecoli_sequence_dict;
    int ordinal_number = 0;
    string sequence_text;
    string ecoli_sequence_file_path = "ecoli/ecoli_simulated_reads.fasta";
    ifstream ecoli_sequence_file(ecoli_sequence_file_path);
    string line;
    while (getline(ecoli_sequence_file, line)) {
        if (line[0] == '>') {
            if (!sequence_text.empty()) {
                ecoli_sequence_dict[ordinal_number] = sequence_text;
                sequence_text.clear();
            }
            ordinal_number++;
        }
    ...
    ofstream output_file_result("ecoli_mutations.txt");
